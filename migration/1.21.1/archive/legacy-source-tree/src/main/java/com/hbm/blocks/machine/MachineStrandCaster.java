...
public class MachineStrandCaster extends BlockDummyable implements ICrucibleAcceptor, ILookOverlay, IToolable {
  ...
  public TileEntity createNewTileEntity(@NotNull World world, int meta) {
    ...
    if (meta >= 6) return new TileEntityProxyCombo(true, false, true, false, true);
    return null;
  }
  ...
  public boolean canAcceptPartialPour(World world, BlockPos pos, double dX, double dY, double dZ, ForgeDirection side, MaterialStack stack) {
    ...
    if (!(poured instanceof TileEntityProxyCombo combo && combo.molten)) return false;
  ...
  public MaterialStack pour(World world, BlockPos pos, double dX, double dY, double dZ, ForgeDirection side, MaterialStack stack) {
    ...
    TileEntity poured = world.getTileEntity(pos);
    if (!(poured instanceof TileEntityProxyCombo combo && combo.molten)) return stack;
  ...
  public boolean onBlockActivated(World world, @NotNull BlockPos pos, @NotNull IBlockState state, @NotNull EntityPlayer player, @NotNull EnumHand hand, @NotNull EnumFacing facing, float hitX, float hitY, float hitZ) {
    ...
    if (caster != null) {
      ...
      if (!heldItem.isEmpty() && heldItem.getItem() == ModItems.mold && caster.inventory.getStackInSlot(0).isEmpty()) {
